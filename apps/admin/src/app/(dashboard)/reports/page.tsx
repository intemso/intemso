...
export default function AdminReportsPage() {
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">Report Review</h1>
        <p className="text-sm text-gray-400 mt-1">{total} total reports</p>
      </div>
            ...
            <div key={r.id} className="bg-gray-800 rounded-xl border border-gray-700 p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white capitalize">{r.reportedEntity} Report</p>
                    <p className="text-xs text-gray-500">Reported by {r.reporter?.email ?? 'Unknown'}</p>
                  </div>
                </div>
                <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${STATUS_STYLES[r.status] || 'bg-gray-700 text-gray-400'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-gray-800 rounded-2xl shadow-xl border border-gray-700 w-full max-w-md p-4 sm:p-6">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setReviewModal(null)} className="px-4 py-2 text-sm text-gray-400 hover:text-white">
                Cancel
              </button>
              <button
