...
const STATUS_STYLES: Record<string, string> = {
  ...
  resolved_student: 'bg-green-50 text-green-700',
  resolved_employer: 'bg-green-50 text-green-700',
  resolved_split: 'bg-teal-50 text-teal-700',
    ...
    <div className="p-6 lg:p-8">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
            ...
            <div key={d.id} className="bg-white rounded-xl border border-gray-100 p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">Contract #{d.contractId.slice(0, 8)}</p>
                    <p className="text-xs text-gray-500">Raised by {d.raisedBy?.email ?? 'Unknown'}</p>
                  </div>
                </div>
                <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${STATUS_STYLES[d.status] || 'bg-gray-100 text-gray-600'}`}>
                  {d.status.replace(/_/g, ' ')}
                </span>
              </div>

              <p className="text-sm text-gray-700 mb-3">{d.reason}</p>

              {d.adminNotes && (
                <div className="bg-gray-50 rounded-lg p-3 mb-3">
                  <p className="text-xs font-medium text-gray-500 mb-1">Admin Notes</p>
                  <p className="text-sm text-gray-700">{d.adminNotes}</p>
                </div>
              )}
          ...
          {disputes.length === 0 && (
            <div className="text-center py-16 text-gray-400 text-sm">No disputes found</div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-4 sm:p-6">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setResolveModal(null)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">
                Cancel
              </button>
              <button
